/* Generic Signal-pattern pipeline player — parameterized version of the
 * Signal Factory front end, used by the Capital Desk demo (and any future
 * pipeline-shaped demo). Vanilla JS only; SSE primary, POST+replay
 * fallback; honors prefers-reduced-motion.
 *
 * Usage: MizokiPipelineDemo.init({
 *   demo: "capital",
...
(function () {
  ...
  function init(config) {
    ...
    function startRun() {
      ...
      var scenario = els.scenario.value || config.defaultScenario;
      ...
      var source = new EventSource(
        config.api + "/stream?scenario=" + encodeURIComponent(scenario) +
        "&seed=" + currentSeed()
      );
      state.source = source;
      var gotFrame = false;
    ...
    function describeItem(stageName, item) {
      ...
      if (stageName === "plan") {
        return item.action_id + " · " + item.type + " " + fmtMag(item) + " → " +
          ...
          (item.headroom_after_pct != null ? ", headroom " + item.headroom_after_pct + "%" : "") + ")";
      }
      if (stageName === "decide") {
  ...
  window.MizokiPipelineDemo = { init: init };
})();
